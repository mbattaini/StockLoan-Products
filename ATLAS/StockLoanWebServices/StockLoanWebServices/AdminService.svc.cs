...
namespace StockLoanWebServices
{
    ...
    public class AdminService : IAdmin
    {
        ...
        //------ Ported from Medalist AdminAgent.cs ----------
        ...
        public byte[] UserRolesGet(short utcOffset)
        {
            string dbCnStr = GetDatabaseConnection("Sendero");  		//new
            SqlConnection dbCn = new SqlConnection(dbCnStr);
        ...
        public byte[] UserRoleFunctionsGet(short utcOffset)
        {
            string dbCnStr = GetDatabaseConnection("Sendero");  		//new
            SqlConnection dbCn = new SqlConnection(dbCnStr);
            DataSet dataSet = new DataSet();

            //DC called once during  client-side RolesGridLoad()
            try
            {
        ...
        public void HolidaysSet(string date, string countryCode, bool isBankHoliday, bool isExchangeHoliday, bool isActive)
        {
            string dbCnStr = GetDatabaseConnection("Sendero");  		//new
            SqlConnection dbCn = new SqlConnection(dbCnStr);
        ...
        public bool HolidayAutoUpdate()
        {
            string dbCnStr = GetDatabaseConnection("Sendero");  		//new
            string syncHolidays = KeyValue.Get("SyncHolidays", "False", dbCnStr);

            try
            {
                return bool.Parse(syncHolidays);
            }
            catch
            {
                Log.Write("The SyncHoliday key value '" + syncHolidays + "' is invalid. [AdminAgent.HolidayAutoUpdate]", Log.Error, 1);
            }

            return false;
        }

        public byte[] BookDataGet(short utcOffset)
        {
            string dbCnStr = GetDatabaseConnection("Sendero");  		//new
            SqlConnection dbCn = new SqlConnection(dbCnStr);
            DataSet dataSet = new DataSet();
