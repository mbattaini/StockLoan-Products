...
namespace StockLoan.TradingData
{
	public class Master
	{
		const string APPLICATION_NAME = "TradingDataServer";
		...
		public Master(int logLevel, string logFilePath)
		{
			...
			try
			{
				TradingData = new TradingData(dbCnStr);
			}
			catch (Exception e)
			{
		...
		public void OnStart()
		{
			...
			KeyValue.Set("TradingDataServiceStartTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCn);
		}
		...
		public void OnStop()
		{
			...
			try
			{
				...
				KeyValue.Set("TradingDataServiceStopTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCn);
			}
			catch (Exception e)
			{
				Log.Write(e.Message + " [Master.OnStop]", Log.Error, 1);
