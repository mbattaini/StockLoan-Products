...
namespace StockLoan.StaticData
{
    public class StaticData
    {
        ...
        public StaticData(string dbCnStr, string externalDbCnStr, string externalPriceDbCnStr)
        {
        ...
        private void CrestPricingLoad()
        {
            ...
            try
            {
                if (Master.BizDate.Equals(DateTime.Now.ToString(Standard.DateFormat)))
                {
                    if (!KeyValue.Get("StaticDataCrestPriceLoadDate", "", dbCnStr).Equals(Master.BizDate))
                    {
                        if (DateTime.Now >= DateTime.Parse(KeyValue.Get("StaticDataCrestPriceLoadTime", "04:30", dbCnStr)))
                        {
                            Log.Write("Loading crest pricing.  [StaticData.CrestPricingLoad]", 2);

                            pricingSql = "select s.sedol,\r\n" +
                                         "       substring(s.isin, 1,2) as countrycode,\r\n" +
                                         "       s.currency,\r\n" +
                                         "       convert(decimal(12, 5), p.offerprice) AS bidprice\r\n" +
                                         "from   dbo.pricefeed p (nolock),\r\n " +
                                         "       dbo.sedl s (nolock)\r\n" +
                                         "where	 p.edate = '" + DateTime.Parse(Master.BizDatePrior).ToString("yyyy-dd-MM") + "'\r\n" +
                                         "and	 s.isin = p.isin\r\n" +
                                         "and	 len(s.sedol) < 8\r\n" +
                                         "and	 len(s.currency) > 0";
                            ...
                            if (dsPrices.Tables["Pricing"].Rows.Count > 0)
                            {
                                ...
                                {
                                    PriceSet(dr["Sedol"].ToString(), dr["CountryCode"].ToString(), dr["Currency"].ToString(), (decimal)dr["bidprice"], Master.BizDate);

                                    i++;

                                    if ((i % 100) == 0)
                                    {
                                        Log.Write("Processed: " + i.ToString("#,##0") + " items. [StaticData.CrestPricingLoad]", 2);
                                    }
                                }

                                Log.Write("Pricing loaded for : " + Master.BizDate + ". [StaticData.CrestPricingLoad]", 2);
                                KeyValue.Set("StaticDataCrestPriceLoadDate", Master.BizDate, dbCnStr);
                            }
                            else
                            {
                                Log.Write("Pricing information unavailable.[StaticData.CrestPricingLoad]", 2);
                            }
                        }
                    }
                    else
                    {
                        Log.Write("Pricing already loaded for : " + Master.BizDate + ". [StaticData.CrestPricingLoad]", 2);
                    }
                }
                else
                {
                    Log.Write("Pricing load must wait for current buisness date. [StaticData.CrestPricingLoad]", 2);
                }
            }
            catch (Exception error)
            {
                Log.Write(error.Message + ". [StaticData.CrestPricingLoad]", 2);
            }
