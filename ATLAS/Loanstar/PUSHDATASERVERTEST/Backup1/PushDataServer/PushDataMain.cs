...
namespace StockLoan.PushData
{
	public class PushData
	{
		...
		public PushData(string dbCnStr, string externalDbCnStr)
		{
		...
		private string CountryCodeConvert(string countryCode)
		{
			...
			{
				Log.Write(error.Message + " [PushData.CountryCodeConvert]", Log.Error, 1);
			}
		...
		public void SecurityMasterLoad()
		{
			...
			try
			{
				...
				{
					if (!KeyValue.Get("PushDataSecurityMasterLoadDate", "", dbCnStr).Equals(Master.BizDate))
					{
						if (DateTime.Now >= DateTime.Parse(KeyValue.Get("PushDataSecurityMasterLoadTime", "04:30", dbCnStr)))
						{
							Log.Write("Loading Secuity Master.  [PushData.SecurityMasterLoad]", 2);
							...
							externalDbCmd.CommandTimeout = int.Parse(KeyValue.Get("PushDataSecurityMasterTimeOut", "600", dbCnStr));
							...
							foreach (DataRow dr in dsSecMaster.Tables["SecurityMaster"].Rows)
							{
								...
								if ((i % 5000) == 0)
								{
									Log.Write("Processed: " + i.ToString("#,##0") + " items. [PushData.SecurityMasterLoad]", 2);
								}
							}

							Log.Write("Security Master loaded for : " + Master.BizDate + ". [PushData.SecurityMasterLoad]", 2);
							KeyValue.Set("PushDataSecurityMasterLoadDate", Master.BizDate, dbCnStr);
						}
					}
					else
					{
						Log.Write("Security Master already loaded for : " + Master.BizDate + ". [PushData.SecurityMasterLoad]", 2);
					}
				}
				else
				{
					Log.Write("Security Master load must wait for current buisness date. [PushData.SecurityMasterLoad]", 2);
				}
			}
			catch (Exception error)
			{
				Log.Write(error.Message + ". [PushData.SecurityMasterLoad]", 2);
			}
		...
		private void CurrencyLoad()
		{
			...
			try
			{
				if (Master.BizDate.Equals(DateTime.Now.ToString(Standard.DateFormat)))
				{
					if (!KeyValue.Get("PushDataCurrencyLoadDate", "", dbCnStr).Equals(Master.BizDate))
					{
						if (DateTime.Now >= DateTime.Parse(KeyValue.Get("PushDataCurrencyLoadTime", "04:30", dbCnStr)))
						{
							Log.Write("Loading Currencies.  [PushData.CurrencyLoad]", 2);
							...
							foreach (DataRow dr in dsCurrencies.Tables["Currency"].Rows)
							{
								...
								if ((i % 5000) == 0)
								{
									Log.Write("Processed: " + i.ToString("#,##0") + " items. [PushData.CurrencyLoad]", 2);
								}
							}

							Log.Write("Currencies loaded for : " + Master.BizDate + ". [PushData.CurrencyLoad]", 2);
							KeyValue.Set("PushDataCurrencyLoadDate", Master.BizDate, dbCnStr);
						}
					}
					else
					{
						Log.Write("Currencies already loaded for : " + Master.BizDate + ". [PushData.CurrencyLoad]", 2);
					}
				}
				else
				{
					Log.Write("Currencies load must wait for current buisness date. [PushData.CurrencyLoad]", 2);
				}
			}
			catch (Exception error)
			{
				Log.Write(error.Message + ". [PushData.CurrencyLoad]", 2);
			}
		...
		private void CurrencyConversionsLoad()
		{
			...
			try
			{
				if (Master.BizDate.Equals(DateTime.Now.ToString(Standard.DateFormat)))
				{
					if (!KeyValue.Get("PushDataCurrencyConversionsLoadDate", "", dbCnStr).Equals(Master.BizDate))
					{
						if (DateTime.Now >= DateTime.Parse(KeyValue.Get("PushDataCurrencyConversionsLoadTime", "04:30", dbCnStr)))
						{
							Log.Write("Loading Currency Conversions.  [PushData.CurrencyConversionsLoad]", 2);
							...
							foreach (DataRow dr in dsCurrencies.Tables["Currency"].Rows)
							{
								...
								if ((i % 5000) == 0)
								{
									Log.Write("Processed: " + i.ToString("#,##0") + " items. [PushData.CurrencyConversionsLoad]", 2);
								}
							}

							Log.Write("Currency Conversions loaded for : " + Master.BizDate + ". [PushData.CurrencyConversionsLoad]", 2);
							KeyValue.Set("PushDataCurrencyConversionsLoadDate", Master.BizDate, dbCnStr);
						}
					}
					else
					{
						Log.Write("Currency Conversion already loaded for : " + Master.BizDate + ". [PushData.CurrencyConversionsLoad]", 2);
					}
				}
				else
				{
					Log.Write("Currency Conversion load must wait for current buisness date. [PushData.CurrencyConversionsLoad]", 2);
				}
			}
			catch (Exception error)
			{
				Log.Write(error.Message + ". [PushData.CurrencyConversionsLoad]", 2);
			}
		...
		private void PriceSet(string secId, string currencyIso, decimal price, string priceDate)
		{
			...
			{
				Log.Write(error.Message + " [PushData.PriceSet]", Log.Error, 1);
			}
			finally
			{
		...
		private void SecMasterItemSet(string secId, string description, string baseType, string classGroup, string countryCode, string currencyIso, string accruedInterest, string recordDateCash, string dividendRate, string secIdGroup, string symbol, string isin, string cusip)
		{
			...
			{
				Log.Write(error.Message + " [PushData.SecMasterItemSet]", Log.Error, 1);
			}
			finally
			{
				if (localDbCn.State != ConnectionState.Closed)
		...
		private void CurrencySet(string currencyIso, string currency, bool isActive)
		{
			...
			{
				Log.Write(error.Message + " [PushData.CurrencySet]", Log.Error, 1);
			}
			finally
			{
				if (localDbCn.State != ConnectionState.Closed)
		...
		private void CurrencyConversionSet(string currencyIsoFrom, string currencyIsoTo, decimal currencyConvertRate)
		{
			...
			{
				Log.Write(error.Message + " [PushData.CurrencyConversionSet]", Log.Error, 1);
			}
			finally
			{
				if (localDbCn.State != ConnectionState.Closed)
