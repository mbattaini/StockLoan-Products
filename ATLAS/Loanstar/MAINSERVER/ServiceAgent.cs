...
namespace StockLoan.Main
{
  ...
  {  
    ...
    public DataSet BookGroupGet(string userId)
    {
        SqlConnection dbCn = new SqlConnection(dbCnStr);
        ...
        try
        {
            ...
            SqlDataAdapter dataAdapter = new SqlDataAdapter(dbCmd);
            dataAdapter.Fill(dsBookGroups, "BookGroups");

            bookGroupList = BookGroupListGet(userId);

            for (int index = 0; index < dsBookGroups.Tables["BookGroups"].Rows.Count; index++)
            {
                if (!bookGroupList.Contains(dsBookGroups.Tables["BookGroups"].Rows[index]["BookGroup"].ToString()))
                {
                    dsBookGroups.Tables["BookGroups"].Rows.RemoveAt(index);
                    index--;
                }
            }

            dsBookGroups.AcceptChanges();
            dsBookGroups.RemotingFormat = SerializationFormat.Binary;
        }
        catch (Exception e)
        {
    ...
    public string BookGroupListGet(string userId)
    {
        SqlConnection dbCn = new SqlConnection(dbCnStr);
        DataSet dataSet = new DataSet();
        string bookGroupList = "";
        ...
        try
        {
            SqlCommand dbCmd = new SqlCommand("spBookGroupListGet", dbCn);
            dbCmd.CommandType = CommandType.StoredProcedure;

            SqlParameter paramUserId = dbCmd.Parameters.Add("@UserId", SqlDbType.VarChar, 50);
            paramUserId.Value = userId;

            SqlParameter paramBookGroupList = dbCmd.Parameters.Add("@BookGroupList", SqlDbType.VarChar, 50);
            paramBookGroupList.Direction = ParameterDirection.Output;

            dbCn.Open();
            dbCmd.ExecuteNonQuery();
            dbCn.Close();

            bookGroupList = paramBookGroupList.Value.ToString();
        }
        catch (Exception e)
        {
            Log.Write(e.Message + " [ServiceAgent.BookGroupListGet]", Log.Error, 1);
            throw;
        }
        finally
        {
	...
	public void PriceSet(string secId, string countryCode, string currencyIso, string price, string priceDate)
	{
		//DC Changed PriceSet method from private to public. 
		//	 Also modified parameters according to dbo.spPirceSet and dbo.tbPrices datatype and Not Null attributes.
		...
		try
		{
			...
			SqlParameter paramPriceDate = dbCmd.Parameters.Add("@PriceDate", SqlDbType.DateTime);
			paramPriceDate.Value = priceDate;	//Note: Systematic XFL Daily Price's DATA Date == BizDatePrior 
		...
		{
		  Log.Write(error.Message + " [ServiceAgent.PriceSet]", Log.Error, 1);
		}
		finally
		{
		  if (localDbCn.State != ConnectionState.Closed)
