...
namespace StockLoan.StaticData
{
	public class Master
	{
		const string APPLICATION_NAME = "StaticDataServer";
		...
		public Master(int logLevel, string logFilePath)
		{
			...
			if (Standard.ConfigValueExists("ExternalMainDatabaseUser")) // User credentials have been specified.
			{
				externalDbCnStr =
				 "User ID=" + Standard.ConfigValue("ExternalMainDatabaseUser") + "; " +
				  "Password=" + Standard.ConfigValue("ExternalMainDatabasePassword") + "; " +
				  "Data Source=" + Standard.ConfigValue("ExternalMainDatabaseHost") + "; " +
				  "Initial Catalog=" + Standard.ConfigValue("ExternalMainDatabaseName") + ";";
			}
			else  // Application user context is trusted, no need for user credentials.
			{
				externalDbCnStr =
            ...
            if (Standard.ConfigValueExists("ExternalPriceDatabaseUser")) // User credentials have been specified.
            {
                externalPriceDbCnStr =
                 "User ID=" + Standard.ConfigValue("ExternalPriceDatabaseUser") + "; " +
                  "Password=" + Standard.ConfigValue("ExternalPriceDatabasePassword") + "; " +
                  "Data Source=" + Standard.ConfigValue("ExternalPriceDatabaseHost") + "; " +
                  "Initial Catalog=" + Standard.ConfigValue("ExternalPriceDatabaseName") + ";";
            }
            else  // Application user context is trusted, no need for user credentials.
            {
                externalPriceDbCnStr =
            ...
            if (Standard.ConfigValueExists("ExternalCurrencyConversionDatabaseUser")) // User credentials have been specified.
            {
                externalCurrencyConversionStr =
                 "User ID=" + Standard.ConfigValue("ExternalCurrencyConversionDatabaseUser") + "; " +
                  "Password=" + Standard.ConfigValue("ExternalCurrencyConversionDatabasePassword") + "; " +
                  "Data Source=" + Standard.ConfigValue("ExternalCurrencyConversionDatabaseHost") + "; " +
                  "Initial Catalog=" + Standard.ConfigValue("ExternalCurrencyConversionDatabaseName") + ";";
            }
            else  // Application user context is trusted, no need for user credentials.
            {
                externalCurrencyConversionStr =
            ...
            Log.Write("Using external price dbCnStr: " + externalPriceDbCnStr + "  [Master.Master]", 2);

			try
			{
				StaticData = new StaticData(dbCnStr, externalDbCnStr, externalPriceDbCnStr, externalCurrencyConversionStr);
			}
			catch (Exception e)
			{
		...
		public void OnStart()
		{
			...
			KeyValue.Set("StaticDataServiceStartTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCn);
		}
		...
		public void OnStop()
		{
			...
			try
			{
				...
				KeyValue.Set("StaticDataServiceStopTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCn);
			}
			catch (Exception e)
			{
				Log.Write(e.Message + " [Master.OnStop]", Log.Error, 1);
