  ...
  public class AdminAgent : MarshalByRefObject, IAdmin
  {
      ...
      string actUserId)
    {
      ...
      try
      {
		...
		{
			SqlParameter paramBookAmountLimits = dbCmd.Parameters.Add("@BorrowLimitAmount", SqlDbType.BigInt);
			paramBookAmountLimits.Value = borrowCreditLimit;
		}

		if (!loanCreditLimit.Equals(""))
		{
			SqlParameter paramLoanAmountLimits = dbCmd.Parameters.Add("@LoanLimitAmount", SqlDbType.BigInt);
    ...
    { 
      ...
      try
      {
        ...
        SqlParameter paramCountryCode = dbCmd.Parameters.Add("@CountryCode", SqlDbType.VarChar, 3);
        if (!countryCode.Equals(""))
        {
          paramCountryCode.Value = countryCode;
        }
