...
namespace StockLoan.PushData
{
	class PdfDoucment
	{
		...
		public void Create(string filePath, string reportName, string header, string columnNames, string body)
		{
			string line = "";
			...
			WriteParagraph(body);
			AddHeaders(reportName, columnNames);
		...
		private void AddHeaders(string reportName, string columnNames)
		{
			Font fontHorz = new Font("Courier New", 8, FontStyle.Bold);
			Font fontColumn = new Font("Courier New", 8, FontStyle.Underline);
			...
			for (int page = 0; page < pdfDocument.Pages.Count; page++)
			{
				...
				rcHeader.Y = rcHeader.Top - 19;
				rcHeader.X = rcHeader.Left;
				rcHeader.Height = 12;

				// add left-aligned footer
				string text = pdfDocument.DocumentInfo.Title;
				pdfDocument.DrawString(text, fontHorz, Brushes.Black, rcHeader);

				// add near text
				text = pdfDocument.DocumentInfo.Title + "Penson Financial Svcs";
				pdfDocument.DrawString(text, fontHorz, Brushes.Black, rcHeader, sfLeft);

				// add center text
				text = reportName + " -" + DateTime.Now.ToString("yyyy-MM-dd");
				pdfDocument.DrawString(text, fontHorz, Brushes.Black, rcHeader, sfCenter);

				// add far text
				text = string.Format("Page {0} of {1}", page + 1, pdfDocument.Pages.Count);
				pdfDocument.DrawString(text, fontHorz, Brushes.Black, rcHeader, sfRight);

				rcHeader.Y = rcHeader.Top + 9;


				text = columnNames;
				pdfDocument.DrawString(text, fontColumn, Brushes.Black, rcHeader, sfLeft);
			}
		}
