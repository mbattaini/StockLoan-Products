...
namespace StockLoan.BDData
{
  ...
  public class Master
  {
    const string APPLICATION_NAME = "BDData";
    ...
    public Master(int logLevel, string logFilePath)
    {
      ...
      try
      { 
        BDDataMain = new BDDataMain(dbCnStr, worldwideDbCnStr);    // Added new argument: WorldWideDbCnStr 
      }
      catch (Exception e)
      {
