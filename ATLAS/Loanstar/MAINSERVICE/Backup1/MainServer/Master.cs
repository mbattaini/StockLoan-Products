	...
	public class Master
	{
		...
		public void OnStart()
		{
			...
			try
			{
				...
				if (bool.Parse(Standard.ConfigValue("UseBackOfficeAgent", "True")))
				{
					backOfficeAgent = (StockLoan.BackOffice.IBackOffice)RemotingTools.ObjectGet(typeof(StockLoan.BackOffice.IBackOffice));

					if (backOfficeAgent == null)
					{
						Log.Write("Remoting config values for the process agent are not correct. [Master.Master]", 1);
					}
				}
				else
				{
					Log.Write("Service will start without implementing the process agent. [Master.Master]", 1);
				}
				...
				positionAgent = new PositionAgent(dbCnStr, ref backOfficeAgent);
