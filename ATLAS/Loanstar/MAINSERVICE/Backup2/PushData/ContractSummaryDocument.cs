				...
				foreach (DataRow contractRow in dsContracts.Tables["Contracts"].Rows)
				{
					if (contractRow["SecId"].ToString().Trim().Equals(secIdRow["SecId"].ToString().Trim()) && contractRow["CurrencyIso"].ToString().Equals(secIdRow["CurrencyIso"].ToString()))
					{
						contractBody += contractRow["BookName"].ToString().PadRight(20, ' ').Substring(0, 20) + spacer +
											   ...
											   decimal.Parse(contractRow["RebateRate"].ToString()).ToString(Formats.Rate).PadRight(RATE_WIDTH, ' ') + spacer;

						if (contractRow["ContractType"].ToString().Equals("B"))
						{
							borrowQuantity = long.Parse(contractRow["Quantity"].ToString()).ToString(Formats.Collateral).PadLeft(QUANTITY_WIDTH, ' ');
							loanQuantity = "0".PadLeft(QUANTITY_WIDTH, ' ');

							borrowAmount = decimal.Parse(contractRow["Amount"].ToString()).ToString(Formats.Cash).PadLeft(AMOUNT_WIDTH, ' ');
						...
						{
							loanQuantity = long.Parse(contractRow["Quantity"].ToString()).ToString(Formats.Collateral).PadLeft(QUANTITY_WIDTH, ' ');
							borrowQuantity = "0".PadLeft(QUANTITY_WIDTH, ' ');

							loanAmount = decimal.Parse(contractRow["Amount"].ToString()).ToString(Formats.Cash).PadLeft(AMOUNT_WIDTH, ' ');
					...
					secIdFooter += "".PadRight(20, ' ') + spacer +
									...
									"".PadRight(15, ' ') + spacer +
									"".PadRight(RATE_WIDTH, ' ') + spacer +
									borrQuantityQ.ToString(Formats.Collateral).PadLeft(QUANTITY_WIDTH, ' ') + spacer +
                                    loanQuantityQ.ToString(Formats.Collateral).PadLeft(QUANTITY_WIDTH, ' ') + spacer +
									borrAmountA.ToString(Formats.Cash).PadLeft(AMOUNT_WIDTH, ' ') + spacer +
                                    loanAmountA.ToString(Formats.Cash).PadLeft(AMOUNT_WIDTH, ' ') + spacer +
									"".PadRight(1, ' ') + " " +
									"".PadRight(4, ' ') + spacer + "#";				
				}
