...
namespace StockLoan.Inventory
{
    ...
    /// </summary>
    public class InventoryImportController : IDisposable
    {

        #region Members

        private ImportMode modeImport;
        ...
        private bool bIsNewModel = false;
        ...
        public static RegexOptions RegExOptions = RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.ExplicitCapture;
        ...
        {
            get { return bIsNewModel; }
            set { bIsNewModel = value; }
        }
        ...
        {
            get { return engineImportViews.SubscriptionTypes; }
        }
        public DataTable ImportDesks
        {
            get { return engineImportViews.ImportDesks; }
        }
        ...
        {
            get { return modelImportSpec.FilePatternTableName; }
        }
        ...
        public void StartImports()
        {
            ...
            if (ImportMode.Service == modeImport || ImportMode.Test == modeImport)
            {
        ...
        public void SavePatternChanges()
        {
            modelImportSpec.UpdateImportSpecs();
        }
        ...
        public void ChangeFilePatternData(long RecordKey, string FieldName, object Value)
        {
            DataTable tableSource = modelImportSpec.ImportDataModel.Tables[FilePatternTableName];
            DataRow rowSource = tableSource.Rows.Find(RecordKey);
            rowSource[FieldName] = Value;
        }
        ...
        public void InitializeComponents()
        {
            try
            {
                if ((null == modelImportSpec) || (!modelImportSpec.HasImportSiteSpecs))
                {
                    ...
                    {
                        SendErrorEmail("Error Initializing Import Controller. Failed to Construct Import Data Model. Please Contact Technical Support.");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void ImportBatchSubscriptionData()
        {
            try
            {
                if (HasImportSiteSpecs)
                {
                    ...
                    Log.Write("Batch Imports Complete. Cleaning Up Data Directories.", Log.Information, 3);
                    int nDirs = CleanUpDataDirectories();
                    Log.Write("Deleted " + nDirs.ToString() + " Directories.", Log.Information, 3);
        ...
        private void ImportServiceSubscriptionData()
        {
            try
            {
                ...
                while (ContinueServiceLoop)
                {
                    if (false == IsNewModel)
                    {
                        try
                        {
                            if (CreateNewImportModel())
                            {
                                CreateNewViewEngine();
                    ...
                    if (HasImportSiteSpecs)
                    {
                        ...
                        Log.Write("Import Service Iteration Complete. ", Log.Information, 3);
                    ...
                    {
                        string strMsgErr = "Inventory Import Model Has Failed to Generate the New ImportSpec object";
                        Log.Write(strMsgErr, Log.Information, 3);
                        UpdateStatusMessage(strMsgErr);
                        WaitThrottle();
                    }
                }
        ...
        internal bool CreateNewImportModel()
        {
            ...
            try
            {
                ...
                modelImportSpec = new InventoryImportModel(modeImport);
                ...
                bReturnSuccess = modelImportSpec.InitializeComponents();
                IsNewModel = modelImportSpec.HasImportSiteSpecs;
            }
            catch (Exception ex)
            {
        ...
        {
            TimeSpan timeWait = GetWaitThrottleTime();
            string strMsgStatus = string.Format("Waiting. Service Loop Loop Will Continue in {0}...", timeWait.ToString());

            UpdateStatusMessage(strMsgStatus);

            Thread.Sleep(timeWait);
        }
        internal TimeSpan GetWaitThrottleTime()
        {
            DateTime dtToday = DateTime.Now.Date;
            TimeSpan timeWaitReturn = TimeSpan.Parse("00:00:10");

            if (dtToday == InventoryImportModel.BizDate)
            {
                timeWaitReturn = InventoryImportModel.WaitTimeBizDate;
            }
            else
            {
                timeWaitReturn = InventoryImportModel.WaitTimeNonBizDate;
            }

            return timeWaitReturn;
        }
        ...
        internal void UpdateStatusMessage(string StatusMessage)
        {
            try
            {
                OperationDescription = StatusMessage;
                            ...
                            if (importSpecFTP.ImportSpecification.IsDownloadSuccessful)
                            {
                                ...
                                if (infoAttachment.Exists)
                                {
                                    ...
                                    if (ScanSpecForDateMatch(importSpecFTP.ImportSpecification))
                                    {
                                        ...
                                        string strMsgStatus = string.Format("Import of SubscriberID: {0} is Complete.", importSpecFTP.ImportSpecification.Desk);
                                        Log.Write(strMsgStatus, Log.Information, 3);

                                        UpdateStatusMessage(engineImportMessages.GenerateStatusMessage(strMsgStatus, importSpecFTP), strMethodName);
                                    }
                                    else
                                    {
                                        if (ImportMode.Batch == modeImport)
                                        {
        ...
        private void ImportEmailData()
        {
            try
            {
                ...
                foreach (DictionaryEntry entry in modelImportSpec.EmailImportSpecs)
                {
                    ...
                    if ((ContinueServiceLoop) && (importSpecEmail.ImportSpecification.IsEnabled))
                    {
                        try
                        {
                            ...
                            if (importSpecEmail.ImportSpecification.IsDownloadSuccessful)
                            {
                                if (importSpecEmail.ImportSpecification.HasImportData)
                                {
                                    ...
                                    if (ScanSpecForDateMatch(importSpecEmail.ImportSpecification))
                                    {
                                        ...
                                        string strMsgStatus = string.Format("Import of SubscriberID: {0} is Complete.", importSpecEmail.ImportSpecification.Desk);
                                        Log.Write(strMsgStatus, Log.Information, 3);

                                        UpdateStatusMessage(engineImportMessages.GenerateStatusMessage(strMsgStatus, importSpecEmail));
                                    }
                                    else
                                    {
                                        if (ImportMode.Batch == modeImport)
                            ...
                            {
                                if (modeImport == ImportMode.Batch)
                                {
                                    string strErr = "";
                                    switch (specActiveImport.SubscriptionType)
                                    {
                                        case "EmailAttachment":
                                            strErr = string.Format("Unable to Locate Email Attachment from Sender: {0} with Subject: {1} and FileName: {2}",
                                            ...
                                            break;
                                        case "EmailBody":
                                            strErr = string.Format("Unable to Locate Email Body from Sender: {0} with Subject: {1}",
                                                                        importSpecEmail.MailAddress,
                                                                        importSpecEmail.MailSubject
                                                                   );
                                            break;
                                        ...
                                        default:
                                            strErr = string.Format("Unable to Locate Email from Sender: {0} with Subject: {1} and FileName: {2}",
                                                                        importSpecEmail.MailAddress,
                                                                        importSpecEmail.MailSubject,
                                                                        importSpecEmail.ImportSpecification.RemoteFilePath
                                                                   );
                                    ...
                                    string strErrEmail = engineImportMessages.GenerateErrorMessage(importSpecEmail, strErr);
                                    SendErrorEmail(strErrEmail);
                                }
                            }
                            specActiveImport = null;
                        }
        ...
        internal void DownloadFTP(FTPImportSpec ImportFTP)
        {
            ...
            try
            {
                StockLoan.Common.Filer FTPFileHandler = new Filer(ImportFTP.ImportSpecification.RootDir);
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(strFileTime))
                    {
                        ...
                        ImportFTP.ImportSpecification.BizDateProposedImport = dtFileTime.ToUniversalTime();
                        ...
                        {
                            string strMsgStatus = string.Format("SubscriberID: {0}; Desk: {1}; Already Has an FTP File With FileTime: {2}, And Will Not Import Data With FileTime: {3}"
                                        , ImportFTP.ImportSpecification.SubscriberID
                                        , ImportFTP.ImportSpecification.Desk
                                        , ImportFTP.ImportSpecification.FileModifiedTime
                                        , dtFileTime.ToUniversalTime().ToString("F")
                                    );
        ...
        internal bool ScanSpecForDateMatch(ImportSpec ImportSpecification)
        {
            ...
            if (RefuseImportOldDates)
            {
                try
                {
                    Log.Write("Comparing Dates", Log.Information, 3);
                    if (!string.IsNullOrEmpty(ImportSpecification.RegexDate))
                    {
                        ...
                        if (ImportSpecification.BizDateProposedImport.Date != dtSystemBizDate.Date)
                        {
                            bReturnMatchSuccess = false;

                            string strMsgErrDate = engineImportMessages.GenerateDateExpectedMessage(ImportSpecification);
                            Console.WriteLine(strMsgErrDate);
                            Log.Write(strMsgErrDate, Log.Information, 3);
                            UpdateStatusMessage(strMsgErrDate);
                            if (ImportMode.Batch == modeImport)
                            {
                        ...
                        if (ImportSpecification.BizDateProposedImport <= ImportSpecification.BizDateLastImported)
                        {
                            bReturnMatchSuccess = false;
                            string strMsgBizDate = String.Format("SubscriberID: {0} Desk: {1} already has data from BizDate: {2} and Will Not Import New Data from BizDate {3}",
                                                                    ImportSpecification.SubscriberID,
                                                                    ImportSpecification.Desk,
                                                                    ImportSpecification.BizDateLastImported,
                                                                    ImportSpecification.BizDateProposedImport
                                                                 );
                            Log.Write(strMsgBizDate, Log.Information, 3);
                            UpdateStatusMessage(strMsgBizDate);
                        }
                    }
                    else
                    {
                        bReturnMatchSuccess = true;
                        string strMsgBizDate = String.Format("SubscriberID: {0} Desk: {1} Has New Data for BizDate: {2} and Will Import New Data and Write Over Data From BizDate {3}",
                                        ImportSpecification.SubscriberID,
                                        ImportSpecification.Desk,
                                        ImportSpecification.BizDateProposedImport,
                                        ImportSpecification.BizDateLastImported
        ...
        {
            return modelImportSpec.GenerateFilePattern(Desk);
        }
        ...
        public static string ParseImportFileDate(string RemoteFilePath, bool IsBizDatePrior)
        {
            ...
            try
            {
                if (RemoteFilePath.Contains("{") && RemoteFilePath.Contains("}"))
                {
                    ...
                    if (@"\" != RemoteFilePath.Substring((nDateStartLocation - 1), 1))
                    {
            ...
            {
                Log.Write(ex.Message + " [InventoryImportController.ParseImportFileDate]", Log.Error, 1);
        ...
        {
            Log.Write("Scanning For Header Data Match", Log.Information, 3);
            ...
            try
            {
                if (!string.IsNullOrEmpty(ImportSpecification.RegexHeader))
                {
                    ...
                    foreach (string strDataRow in RowData)
                    {
                        ...
                        {
                            Log.Write("Found Header Data Match !", Log.Information, 3);
                            break;
                        }
                    }
                }
            ...
            if (!bReturnMatchSuccess) { Log.Write("Header Data Not Found!", Log.Information, 3); }
            return bReturnMatchSuccess;
        }
        ...
        private int CleanUpDataDirectories()
        {
            int nReturnNumDeleted = 0;
            try
            {
                Log.Write("Cleaning Up Data Directories.", Log.Information, 3);

                string strRootDir = StockLoan.Common.Standard.ConfigValue("InventoryLocalDestination");

                string strSubDirFTP = StockLoan.Common.Standard.ConfigValue("InventoryFTPSubDir");
                string strSubDirEmail = StockLoan.Common.Standard.ConfigValue("InventoryEmailSubDir");

                string strPathFTP = strRootDir + strSubDirFTP;
                string strPathEmail = strRootDir + strSubDirEmail;

                nReturnNumDeleted += CleanUpDataDirectory(strPathFTP);
                nReturnNumDeleted += CleanUpDataDirectory(strPathEmail);

                Log.Write("Deleted " + nReturnNumDeleted.ToString() + " Directories.", Log.Information, 3);
            }
            catch(Exception ex)
            {
                Log.Write(ex.Message + " [InventoryImportController.SendErrorEmail]", Log.Error, 1);
            ...
            return nReturnNumDeleted;
        }

        private int CleanUpDataDirectory(string DirectoryPath)
        {
            int nReturnNumDeleted = 0;
            try
            {
                TimeSpan tsInventoryDataEndurance = InventoryImportModel.InventoryDataEndurance;
                DirectoryInfo infoDirInventoryData = new DirectoryInfo(DirectoryPath);

                if (infoDirInventoryData.Exists)
                {
                    DirectoryInfo[] arrayInfoSubDirectories = infoDirInventoryData.GetDirectories();
                    for (int iDir = 0; iDir < arrayInfoSubDirectories.Length; iDir++)
                    {
                        DirectoryInfo infoDirCurrentSub = arrayInfoSubDirectories[iDir];
                        DateTime dtCreateDateUTC = infoDirCurrentSub.CreationTimeUtc;
                        DateTime dtNowUTC = DateTime.Now.ToUniversalTime();
                        TimeSpan tsDirAge = dtNowUTC - dtCreateDateUTC;
                        if (tsInventoryDataEndurance < tsDirAge)
                        {
                            nReturnNumDeleted++;
                            infoDirCurrentSub.Delete(true);
                        }
                    }
                }
            }
