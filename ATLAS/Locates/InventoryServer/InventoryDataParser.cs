                        ...
                        if (ImportSpecification.HasImportData)
                        {
                            ...
                            foreach (string strRow in arrayStrFileRows)
                            {
                                if ((null != strRow) && (string.Empty != strRow.Trim()))
                                {
                                    ...
                                    // Find Date (Find Once)
                                    if ((null != rgxDateCol) && (1900 == ImportSpecification.BizDateProposedImport.Year) && rgxDateCol.IsMatch(strRow))
                                    {
                                        DateTime dtSpecified = ParseDateText(ImportSpecification, strRow);

                                        // This Date Is Used by the InventoryDataEntry object, and ends up in tbInventory.
                                        // This Value will be used to determine whether to UPDATE a record for the same day, or INSERT a new record.
                                        ImportSpecification.BizDateProposedImport = dtSpecified.ToUniversalTime();
                                    }
                            ...
                            foreach (InventoryDataEntry ActiveInventoryEntry in ImportSpecification.InventoryEntryObjects)
                            {
                                ...
                                //Set Date
                                if (1900 != ImportSpecification.BizDateProposedImport.Year)
                                {
                                    ActiveInventoryEntry.BizDate = ImportSpecification.BizDateProposedImport;
                                }
                                else
                                {
            ...
            if (!string.IsNullOrEmpty(ImportSpecification.RegexDate))
            {
                ...
                if (matchDateCol.Success)
                {
                    try
                    {
                        ...
                        string strHour = "12";
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Hour"].Value))
                            {strHour = matchDateCol.Groups["Hour"].Value;}

                        string strMinute = "00";
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Min"].Value))
                            {strMinute = matchDateCol.Groups["Min"].Value;}
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Minute"].Value))
                            { strMinute = matchDateCol.Groups["Minute"].Value; }

                        string strSecond = "00";
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Sec"].Value))
                            { strSecond = matchDateCol.Groups["Sec"].Value; }
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Second"].Value))
                            { strSecond = matchDateCol.Groups["Second"].Value; }

                        string strMeridiam = "AM";
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Meridiam"].Value))
                            { strMeridiam = matchDateCol.Groups["Meridiam"].Value; }
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["Suffix"].Value))
                            { strMeridiam = matchDateCol.Groups["Suffix"].Value; }
                        if (!string.IsNullOrEmpty(matchDateCol.Groups["AMPM"].Value))
                            { strMeridiam = matchDateCol.Groups["AMPM"].Value; }
                        ...
                        if (12 < nHour)
                        {
                            strBizdate = string.Format("{0}/{1}/{2} {3}:{4}:{5}", strMonth, strDay, strYear, strHour, strMinute, strSecond );                            
                        }
                        else
                        {
                            strBizdate = string.Format("{0}/{1}/{2} {3}:{4}:{5} {6}", strMonth, strDay, strYear, strHour, strMinute, strSecond, strMeridiam);                            
                        }
