...
Declare	@BizDate datetime,
	@BizDateCount int,
	@EasyBorrowQuantityHaircut varchar(10),
	@EasyBorrowQuantityMinimum varchar(10),
	@EasyBorrowSourceList varchar(250),
	@EasyBorrowSecIdList varchar(250)
	
Select	@EasyBorrowQuantityHaircut = '1.0'
Exec	spKeyValueGet 'EasyBorrowQuantityHaircut', @EasyBorrowQuantityHaircut Output

Select	@EasyBorrowQuantityMinimum = '350000'
Exec	spKeyValueGet 'EasyBorrowQuantityMinimum', @EasyBorrowQuantityMinimum Output

Select	@EasyBorrowSourceList = 'Desk list delimited by [;]'
Exec	spKeyValueGet 'EasyBorrowSourceList', @EasyBorrowSourceList Output

Select	@EasyBorrowSecIdList = 'Sec ID list delimited by [;]'
Exec	spKeyValueGet 'EasyBorrowSecIdList', @EasyBorrowSecIdList Output
...
Begin
	...
	Where		CharIndex(Desk + ';', @EasyBorrowSourceList) > 0
...
-- Purge existing records if this is a re-run.
Delete
From	tbBorrowEasy
...
Select		@TradeDate,
		I.SecId,
		Convert(bigint, Sum(I.Quantity * Convert(float, @EasyBorrowQuantityHaircut))) As Quantity
From		tbInventory I,
...
Where		I.BizDate = IC.BizDate
	...
	And	I.Quantity >= Convert(bigint, @EasyBorrowQuantityMinimum)
	And	((CharIndex(I.Desk + ';', @EasyBorrowSourceList) > 0) And (I.ModeCode = 'F'))
...
Select		@TradeDate,
		SecId,
		Convert(bigint, @EasyBorrowQuantityMinimum) As Quantity
From		tbSecMaster
Where		(CharIndex(SecId + ';', @EasyBorrowSecIdList) > 0)
...
Select		*
From		tbSecMaster
Where		SecId In (Select SecId From tbSecIdLinks Where SecIdLink = @SecID)
...
CREATE Procedure dbo.spDeskQuipGet
	...
	@DayCount smallint = 10,
	@UtcOffset smallint = 0 
As
Begin

Declare	@ActTimeMin datetime
Select	@ActTimeMin = Convert(char(10), DateAdd(d, - @DayCount, GetUtcDate()), 120)

Select		DQ.SecId,
		SIL.SecIdLink As Symbol,
		DQ.DeskQuip,
		U.ShortName As ActUserShortName,
		DateAdd(n, @UtcOffset, DQ.ActTime) As ActTime
From		tbDeskQuips DQ,
		tbSecIdLinks SIL,
		tbUsers U
...
If Not Exists (Select * From tbFunctions Where FunctionPath = 'AdminBooks')
	Insert Into tbFunctions Values ('AdminBooks','0','0')
GO

If Not Exists (Select * From tbFunctions Where FunctionPath = 'PositionOpenContracts')
	Insert Into tbFunctions Values ('PositionOpenContracts','0','0')
GO
...
Select		B.BookGroup,
		...
		IsNull(AP.BorrowAmount, 0) As AmountBorrowToday,
		IsNull(AP.LoanAmount, 0) As AmountLoanToday,
		IsNull(AP.BorrowAmount, 0) + IsNull(AP.LoanAmount, 0) As AmountTotalToday,
		B.Comment,
		U.ShortName As ActUserShortName,
		B.ActTime
From		tbBooks B,
...
If Not Exists (Select * From tbFunctions Where FunctionPath = 'PositionDealBlotter')
	Insert Into tbFunctions Values ('PositionDealBlotter', '0', '0')
GO
...
Set	Status = 'C'
Where	Status = 'O'
And	RecallId Not In (Select RecallId From #RecallPosition)

-- Remove recalls from the Loanet set known in the local set.
Delete
From	#RecallPosition
Where	RecallId In (Select RecallId From tbRecalls Where BizDate = @BizDate)
...
If Not Exists (Select * From tbFunctions Where FunctionPath = 'PositionRecalls')
	Insert Into tbFunctions Values ('PositionRecalls', '0', '0')
GO
...
Declare @CurrentBizDate datetime
Select  @CurrentBizDate = (Select KeyValue From tbKeyValues Where KeyId = 'ContractsBizDate')
...
Select 		R.RecallId,
		...
		Case When @BizDate < @CurrentBizDate  Then 'H' Else R.Status End As Status
From		tbRecalls R,
		tbRecallReasons RR,
		tbUsers U,
		tbSecIdLinks SIL,		
...
Where	Status = 'M'
Or	Status = 'O'
And	IsNull(MoveToDate, BaseDueDate) <= @BizDate
...
If Not Exists (Select * From tbFunctions Where FunctionPath = 'PositionContractBlotter')
	Insert Into tbFunctions Values ('PositionContractsNew', '0', '0')
GO
...
If Not Exists (Select FunctionPath From tbFunctions Where FunctionPath = 'PositionBoxSummary')
	Insert Into tbFunctions Values ('PositionBoxSummary', '0', '0')
GO
...
Select		R.BookGroup,
		...
		R.BorrowRecalls,
		R.LoanRecalls,
		Convert(bigint, 0) As Available,
		Convert(bigint, 0) As AvailableAmount,
		Convert(bigint, 0) As PledgeUndo,
		Convert(bigint, 0) As Borrow,
		Convert(bigint, 0) As BorrowAmount,
		Convert(bigint, 0) As Recall,
...
If Not Exists (Select * from tbFunctions Where FunctionPath = 'InventoryPublisher')
	Insert Into tbFunctions Values ('InventoryPublisher','0','0')
GO
