	...
	{    
		...
		private void MedalistMainLoop()
		{
			while (!isStopped) // Loop through this block (otherwise exit method and thread dies).
			{
				...
				//DChen, generate daily Automated Borrow Return Input file (filename: DBI0234)
				AutomatedBorrowReturnFileDo();
				if (isStopped) break;*/
		...
		private void AutomatedBorrowReturnFileDo()
		{				
			int		rowCount = 0;  
			...
			string localFilePath =  MedalistMain.TempPath;
			string remotePathName = Standard.ConfigValue("LoanetAutomatedBorrowReturnPathName", "");	
			string remoteFileName = Standard.ConfigValue("LoanetAutomatedBorrowReturnFileName", "");	
			string remoteFtpHost = Standard.ConfigValue("LoanetHostName", "");
			string remoteFtpUserName = Standard.ConfigValue("LoanetUserName", "");
			string remoteFtpPassword = Standard.ConfigValue("LoanetPassword", "");

			
			if (!Master.ContractsBizDate.Equals(Master.BizDate))
			{	
				Log.Write("Not current BizDate, need not create the Automated Borrow Return File. ContractsBizDate: " + Master.ContractsBizDate + ", BizDate: " + Master.BizDate + ".  [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
				return; 
			}

			if (!KeyValue.Get("S3StartOfDayDate", "", dbCnStr).Equals(Master.ContractsBizDate))
			{	
				Log.Write("S3 Start of Day is not ready, skip creating the Automated Borrow Return File. ContractsBizDate: " + Master.ContractsBizDate + ", S3StartOfDayDate: " + KeyValue.Get("S3StartOfDayDate", "", dbCnStr) + ".  [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
				return; 
			}

			dayTime = KeyValue.Get("AutomatedBorrowReturnStartTime", "06:00", dbCnStr);

			if (dayTime.CompareTo(DateTime.Now.ToString("HH:mm")) >= 0)
			{
				Log.Write("Not ready to run for today, must wait until after " + dayTime + " to run. [MedalistMain.AutomatedBorrowReturnFileDo]", Log.Error, 1);
				return; 
			}

			if (localFilePath.Equals("") || remoteFileName.Equals("") || remoteFtpHost.Equals("") || remoteFtpUserName.Equals("") || remoteFtpPassword.Equals(""))
			{	
				Log.Write("The configuration value for Loanet ftp host is missing, skip uploading Automated Borrow Return file for " + Master.ContractsBizDate + ". [MedalistMain.AutomatedBorrowReturnFileDo]", Log.Error, 1);
				return; 
			}

			if (!KeyValue.Get("AutomatedBorrowReturnFileDate", "", dbCnStr).Equals(Master.ContractsBizDate))
			{
				Log.Write("Will do the Automated Borrrow Return file for " + Master.ContractsBizDate + ". [MedalistMain.AutomatedBorrowReturnFileDo]",3);
				...
				{	
					...
					dbCmd = new SqlCommand("spBoxSummaryShortGet", dbCn); 
					...
					dataAdapter.Fill(dataSet, "BoxSummaryShort");
					
					dataSet.Tables["BoxSummaryShort"].PrimaryKey = new DataColumn[1]
					{	dataSet.Tables["BoxSummaryShort"].Columns["SecId"]	}; 
														
					dbCmd.CommandText = "spContractGet";
					dbCmd.CommandType = CommandType.StoredProcedure;


					SqlParameter paramContractType = dbCmd.Parameters.Add("@ContractType", SqlDbType.Char);
					paramContractType.Value = "B";

					dataAdapter.Fill(dataSet, "Contracts");
					
					
					streamWriter = new StreamWriter(localFilePath + remoteFileName, false, System.Text.Encoding.ASCII);

					streamWriter.Write("1" + "0234" + DateTime.Parse(Master.ContractsBizDate).ToString("MMddyy") + "BORR" + "    "); 										
					
					long borrowQuantity = 0;
					long exDeficitSettled = 0;
	
					foreach (DataRow dataRowBSS in dataSet.Tables["BoxSummaryShort"].Rows)
					{	
						exDeficitSettled = 0;
						borrowQuantity = 0;

						foreach (DataRow dataRow in dataSet.Tables["Contracts"].Select("SecId = '" + dataRowBSS["SecId"] + "'", "", DataViewRowState.CurrentRows))
						{
							borrowQuantity = borrowQuantity + (long)dataRow["Quantity"];						
						}
					
						exDeficitSettled = (long) dataRowBSS["ExDeficitSettled"];
						...
						if (exDeficitSettled > borrowQuantity)
						{
							exDeficitSettled = borrowQuantity;
						}

						exDeficitSettled = (exDeficitSettled - (exDeficitSettled % 100));
			
						detailString += dataRowBSS["SecId"].ToString().Trim().PadLeft(9,' ').Substring(0,9); 

						detailString += exDeficitSettled.ToString().Trim().Replace("-","").PadLeft(9,'0').Substring(0,9); 
			
						streamWriter.Write(detailString); 

						rowCount++; 
						totalQuantity += long.Parse(exDeficitSettled.ToString().Trim().Replace("-","").PadLeft(9,'0').Substring(0,9)); 
						detailString = "";
					}
					
					streamWriter.Write("9" + rowCount.ToString().Trim().PadLeft(5,'0').Substring(0,5) + totalQuantity.ToString().Trim().PadLeft(11,'0').Substring(0,11) + "  "); 

					streamWriter.Flush();
					streamWriter.Close(); 
					Log.Write("Automated Borrrow Return file " + remoteFileName + " generated for " + Master.ContractsBizDate + ". [MedalistMain.AutomatedBorrowReturnFileDo]",3);

					Anetics.Common.Filer filer; 
					filer = new Anetics.Common.Filer(localFilePath); 

					Log.Write("Sending ABR file " + MedalistMain.tempPath + remoteFileName + " to " +  remoteFtpHost + remotePathName + ". [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
					filer.FilePut(remotePathName + remoteFileName, remoteFtpHost, remoteFtpUserName, remoteFtpPassword, localFilePath + remoteFileName);
					
					Log.Write("Automated Borrrow Return file generated and uploaded for " + Master.ContractsBizDate + ". [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
					KeyValue.Set("AutomatedBorrowReturnFileDate", Master.ContractsBizDate, dbCn); 
					KeyValue.Set("AutomatedBorrowReturnFileUploadDate", Master.ContractsBizDate, dbCn); 
				}
				catch (Exception e)
				{
					Log.Write(e.Message + "  [MedalistMain.AutomatedBorrowReturnFileDo]", Log.Error, 1);
					throw;
				}
				finally
				{
			...
			{
				Log.Write("Automated Borrow Return File (" + remoteFileName + ") has already been created for: " + KeyValue.Get("AutomatedBorrowReturnFileDate", "", dbCnStr) + " [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
				
				if (KeyValue.Get("AutomatedBorrowReturnFileUploadDate", "", dbCnStr).Equals(Master.ContractsBizDate))
				{
					Log.Write("Automated Borrow Return File (" + remoteFileName + ") has already been uploaded (ftp) to Loanet for: " + KeyValue.Get("AutomatedBorrowReturnFileUploadDate", "", dbCnStr) + " [MedalistMain.AutomatedBorrowReturnFileDo]", 3);
				}
			}		
		}
