...
namespace Anetics.Common
{
  ...
  public class Filer
    ...
    /// </summary>
		...
		public void FileNoTempPut(string remotePathName, string hostName, string userName, string password, string localPathName)
		{
			if (hostName.ToLower().Trim().Equals("localhost")) // LocalHost so do not use FTP.
			{
				File.Copy(localPathName, remotePathName, true);
			...
			else
			{
				try
				{
					DoConnect(hostName, userName, password);
					ftp.SetTransferType(Rebex.Net.FtpTransferType.Binary);
					ftp.PutFile(localPathName, remotePathName);
