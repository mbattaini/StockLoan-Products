...
namespace Anetics.Penson
{
  public class Master
  {
    const string APPLICATION_NAME = "Penson";
    
    private string dbCnStr = "";
    private string dbCnStrPenson = "";
    private PensonMain pensonMain;
    ...
    public Master(int logLevel, string logFilePath)
    {
      ...
      {
        dbCnStr =  
          "User ID=" + Standard.ConfigValue("MainDatabase.User") + "; " + 
          "Password=" + Standard.ConfigValue("MainDatabase.Password") + "; " +
          "Data Source=" + Standard.ConfigValue("MainDatabase.Host") + "; " + 
          "Initial Catalog=" + Standard.ConfigValue("MainDatabase.Name") + ";";
      }
      ...
      if (Standard.ConfigValueExists("PensonDatabase.User")) // User credentials have been specified.
      {
        dbCnStrPenson =  
          "User ID=" + Standard.ConfigValue("PensonDatabase.User") + "; " + 
          "Password=" + Standard.ConfigValue("PensonDatabase.Password") + "; " +
          "Data Source=" + Standard.ConfigValue("PensonDatabase.Host") + "; " + 
          "Initial Catalog=" + Standard.ConfigValue("PensonDatabase.Name") + ";";
      }
      else  // Application user context is trusted, no need for user credentials.
      {
        dbCnStrPenson = 
      ...
      Log.Write("Using dbCnStrPenson: " + dbCnStrPenson + " [Master.Master]", 2);

      try
      { 
        pensonMain = new PensonMain(dbCnStr, dbCnStrPenson);
      }
      catch (Exception e)
      {
    ...
    public void OnStart()
    {
      ...
      KeyValue.Set("PensonServiceStartTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCnStr);
    }
    ...
    public void OnStop()
    {
      ...
      KeyValue.Set("PensonServiceStopTime", DateTime.UtcNow.ToString(Standard.DateTimeFormat), dbCnStr);
    }
  }
}
